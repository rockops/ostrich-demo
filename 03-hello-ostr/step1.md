...
```bash
curl -L -o /usr/local/bin/ostr "https://github.com/rockops/ostrich-sdk/releases/latest/download/ostr-linux-amd64"
chmod +x /usr/local/bin/ostr
```{{exec}}

### Help command

The `ostr` CLI is the remmote CLI for Ostrich SDK.

Get general help and list main operations for the `ostr` command:
...
Create a connection (to be done once):

```bash
ostr init
```{{exec}}

- SDK endpoint: `172.30.1.2`
- Config name: `local` (you can change it to any other name)
- Port: `31022` (just press ENTER to accept the default value)
- Password: `ostrich@sdk`
...
You can see the endpoint you have just created. You could also add new endpoint to connect to different clusters running Ostrich SDK, by using the `ostr init` command again. 
...
```bash
ostr template install ostrich/hello
```{{exec}}
...
cd /root/hello-test
ostr template config hello > ostrich.yaml
```{{exec}}
